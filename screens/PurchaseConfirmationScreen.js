...
const USER_PLAN = "premium"; // "free", "premium", or "elite"
...
const productName = item?.title || item?.name || "Legacy Item";
  const rawPrice = item.price || item.retailPrice || item.amount || 0;

const price =
  typeof rawPrice === "number"
    ? rawPrice
    : Number(String(rawPrice).replace("$", "").trim()) || 0;
...
const planDiscountRate =
  USER_PLAN === "elite" ? 0.15 :
  USER_PLAN === "premium" ? 0.10 :
  0;
const planDiscount = price * planDiscountRate;
const planName =
  USER_PLAN === "elite" ? "Elite" :
  USER_PLAN === "premium" ? "Premium" :
  "Free";

const membershipDiscount = price * planDiscountRate;
const wCoinDiscount = coins > 0 ? Math.min(price * 0.15, coins / 100) : 0;
const finalPrice = Math.max(price - membershipDiscount - wCoinDiscount, 0);
      ...
      >
        <View style={styles.successCard}>
          ...
          <Text style={styles.successSub}>
            Thank you for supporting Legacy Walk. Your order has been
            successfully processed.
          </Text>

          <View style={styles.orderBox}>
            {item.image && (
              <Image source={item.image} style={styles.confirmImage} />
            )}

            <Info label="Item Price" value={`$${price.toFixed(2)}`} />
<Info label="Membership" value={`${planName} Plan`} />
<Info label="Membership Discount" value={`-$${membershipDiscount.toFixed(2)}`} />
<Info label="W Coin Discount" value={`-$${wCoinDiscount.toFixed(2)}`} />
<Info label="Shipping" value="Free" />
<Info label="Estimated Delivery" value="3–5 Business Days" />

            <View style={styles.checkList}>
              <Text style={styles.checkText}>✓ Inventory Updated</Text>
              <Text style={styles.checkText}>✓ Reward History Recorded</Text>
              <Text style={styles.checkText}>✓ W Coin Discount Applied</Text>
            </View>
          </View>
